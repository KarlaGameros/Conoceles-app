onload = (function () {
    ...
    if (event.target.id == "activar-voz") {
      ...
      if (contenido.length > 0) {
        for (var i = 0; i < contenido.length; i++) {
          ...
          const btnPlayParrafo = document.querySelector(
            `#play${botones.length + i}`
          );
          const btnPauseParrafo = document.querySelector(
            `#pause${botones.length + i}`
          );
          const btnStopParrafo = document.querySelector(
  ...
  function onClickActivarSeleccionVoz(event) {
    ...
    if (
      event.target.id == "activarSeleccion-voz" &&
      getComputedStyle(desacSelecVoz).display === "block"
    ) {
      var synth = speechSynthesis;
      ...
      function selectableTextAreaMouseUp(event) {
        setTimeout(() => {
          ...
          if (selectedText.length) {
            ...
            const btnPlayWidth = Number(
              getComputedStyle(btnplay).width.slice(0, -2)
            );
            const btnPlayHeight = Number(
              getComputedStyle(btnplay).height.slice(0, -2)
            );
            const btnPauseWidth = Number(
              getComputedStyle(btnpause).width.slice(0, -2)
            );
            const btnPauseHeight = Number(
              getComputedStyle(btnpause).height.slice(0, -2)
            );
            const btnStopWidth = Number(
              getComputedStyle(btnstop).width.slice(0, -2)
            );
            const btnStopHeight = Number(
              getComputedStyle(btnstop).height.slice(0, -2)
            );

            if (
              document.activeElement !== btnplay &&
              document.activeElement !== btnpause &&
              document.activeElement !== btnstop
            ) {
