<template>
  <q-layout :view="$q.screen.xs ? 'hHh lpR lff' : 'hHh LpR fFf'">
    ...
    ">
        ...
        <div v-if="!$q.screen.xs" class="col-lg-2 col-md-2 col-sm-3 col-xs-3 q-pr-xl">
          <q-img src="../assets/Conoceles2@300x.png"></q-img>
        </div>
      </div>
      <q-toolbar>
        <div class="row">
          <q-btn v-if="isHomePage || $q.screen.xs" dense class="absolute-left" flat round icon="menu"
          ...
          <div v-if="!$q.screen.xs" class="absolute-center col-lg-8 col-md-8 col-sm-8 col-xs-9">
            <q-tabs v-model="tab">
              ...
              <q-tab id="speak" v-model="tab" v-for="eleccion in list_Tipos_Elecciones" :key="eleccion.siglas"
                :name="eleccion.siglas" :label="eleccion.nombre" @click="isTabSelected(eleccion.siglas, eleccion.id)" />
            </q-tabs>
          </div>
          <!---------------------------HEADER ISSMALLSREEN--------------------------->
    ...
    tab == "DIP"
      ...
      : tab == "PYS"
        ...
        : tab == "REG"
          ...
          <div v-if="!$q.screen.xs && !$q.screen.sm" class="absolute-right q-pa-xs" id="speak">
    ...
    <q-drawer v-if="isHomePage || $q.screen.xs" show-if-above v-model="leftDrawerOpen" side="left" bordered :class="{
        ...
        <q-list v-if="!$q.screen.xs">
          <q-item>
            <q-item-section class="bg-grey-3" style="border-radius: 10px">
              <q-item-label class="text-h5 label-title text-bold q-pa-xs" style="color: gray" id="speak">Consulta
              </q-item-label>
              <div class="text-subtitle2 q-pa-xs" style="color: grey" id="speak">
                Selecciona el reporte que deseas consultar:
              </div>
              <q-btn rounded @click="activaNumeralia(tab)" :to="{
    name: `${tab}`,
    params: { eleccion_Id: tab_Id },
  }" label="Numeralia" :class="buttons == false ? 'bg-pink-ieen' : 'bg-pink-ieen-3'" />
              <div class="q-pb-xs"></div>
              <q-btn rounded :to="{
    name: `${tab}cards`,
    params: { eleccion_Id: tab_Id },
  }" label="Candidatas y candidatos" :class="buttons ? 'bg-pink-ieen' : 'bg-pink-ieen-3'"
          ...
          <q-item v-if="tab != 'PYS'">
            <q-item-section>
              ...
              <q-select dense color="purple" rounded outlined bg-color="grey-3" v-model="cargo_Id"
                :options="list_Cargos" />
            </q-item-section>
          </q-item>
          <q-item v-if="tab === 'DIP'" :content-inset-level="2" :header-inset-level="2">
            <q-item-section>
              <q-item-label class="text-purple-ieen label-title text-bold" id="speak">Distrito</q-item-label>
              <q-select :disable="cargo_Id.value == 0 || cargo_Id.cargo == 'RP'" dense color="purple" rounded outlined
                bg-color="grey-3" v-model="distrito_Id" :options="list_Distritos" />
            </q-item-section>
          </q-item>
          <q-item v-if="tab === 'PYS' || tab === 'REG'" :content-inset-level="2" :header-inset-level="2">
            <q-item-section>
              <q-item-label class="text-purple-ieen label-title text-bold" id="speak">Municipio</q-item-label>
              <q-select :disable="cargo_Id.value == 0" dense color="purple" outlined rounded bg-color="grey-3"
                v-model="municipio_Id" :options="list_Municipios" />
            </q-item-section>
          </q-item>
          <q-item v-if="tab === 'REG'" :content-inset-level="2" :header-inset-level="2">
            <q-item-section>
              <q-item-label class="text-purple-ieen label-title text-bold" id="speak">Demarcación</q-item-label>
              <q-select :disable="cargo_Id.value == 0 || cargo_Id.cargo == 'RP'" dense color="purple" outlined
                bg-color="grey-3" rounded v-model="demarcacion_Id" :options="list_Demarcaciones" />
            </q-item-section>
          </q-item>
          <q-item>
            <q-item-section>
              <q-item-label class="text-purple-ieen label-title text-bold" id="speak">Partido político o
                Coalición</q-item-label>
              <q-select :disable="cargo_Id.value == 0" dense color="purple" rounded outlined bg-color="grey-3"
                v-model="actor_politico_Id" :options="list_Partidos_Politicos">
          ...
          <q-item>
            <q-item-section>
              <q-item-label class="text-purple-ieen label-title text-bold" id="speak">Grado académico</q-item-label>
              <q-select :disable="cargo_Id.value == 0" dense color="purple" rounded outlined bg-color="grey-3"
                v-model="grado_Academica_Id" :options="list_Grado_Academico" />
            </q-item-section>
          </q-item>
          <q-item>
            <q-item-section>
              <q-item-label class="text-purple-ieen label-title text-bold" id="speak">Rango de edad</q-item-label>
              <q-select :disable="cargo_Id.value == 0" dense color="purple" rounded outlined bg-color="grey-3"
                v-model="rango_edad_Id" :options="list_Edades" />
            </q-item-section>
          </q-item>
          <q-item>
            <q-item-section>
              <q-item-label class="text-purple-ieen label-title text-bold" id="speak">Género</q-item-label>
              <q-select :disable="cargo_Id.value == 0" dense color="purple" rounded outlined bg-color="grey-3"
                v-model="sexo_Id" :options="list_Sexo" />
            </q-item-section>
          </q-item>
        </q-list>
        ...
        <q-list v-else class="absolute">
          ...
          <q-item clickable v-ripple>
            <q-btn @click="isTabSelected('INICIO')" rounded name="inicio" icon="home" to="/inicio" label="Inicio"
              class="bg-pink-ieen-3" />
          </q-item>
          <q-item v-for="eleccion in list_Tipos_Elecciones" :key="eleccion" clickable v-ripple>
            <q-btn v-model="tab" @click="isTabSelected(eleccion.siglas, eleccion.id)" rounded :to="{
    name: eleccion.siglas,
  }" :label="eleccion.label" class="bg-pink-ieen-3" />
          </q-item>
        </q-list>
      </q-scroll-area>
      <q-page-sticky v-if="!$q.screen.xs" :offset="[18, 18]">
        <div class="row justify-center">
          ...
          <div>
            <q-btn dense :disable="cargo_Id.value == 0" @click="consultar" push class="bg-pink-ieen">
              <div class="row items-center no-wrap">
        ...
        <q-toolbar-title>
          ...
          <div>
            ...
            <div class="container-voice">
              ...
              <div class="Acces-voz">
                <button id="activar-voz" class="fa fa-eye desactivado activar-voz" style="display: block"
                  onClick="openToastr('Lectura por Parrafos Activada')"></button>
                <button id="desactivar-voz" class="fa fa-eye-slash activado" style="display: none"
                  onClick="openToastr('Lectura por Parrafos Desactivada')"></button>
                <button id="activarSeleccion-voz" class="fa fa-mouse-pointer desactivado" style="display: block"
                  onClick="openToastr('Lectura por Selección Activada')"></button>
                <button id="desactivarSeleccion-voz" class="fa fa-times activado" style="display: none"
                  onClick="openToastr('Lectura por Selección Desactivada')"></button>
              </div>
              <div class="btn-mas">
    ...
    " id="speak">
            ...
            <div class="col-lg-3 col-md-3 col-sm-6 col-xs-12 text-center text-caption">
              <div><q-icon name="home" color="white" /><span id="speak">Domicilio</span></div>
              <span id="speak">Av. Country Club 13, Colonia Versalles, 63138, Tepic, Nayarit</span>
            </div>
            <div class="text-caption col-lg-3 col-md-3 col-sm-6 col-xs-12 text-center">
              <div><q-icon name="phone" color="white" />
            ...
            <div v-if="!$q.screen.xs" class="absolute-right q-pa-xs">
              <q-btn flat round dense href="https://www.facebook.com/IEENayarit?mibextid=sCpJLy" target="_blank">
                <i class="fa-brands fa-square-facebook fa-2xl" style="color: #ffffff"></i>
              </q-btn>
              <q-btn flat round dense href="https://www.youtube.com/@IEENayarit" target="_blank">
  ...
  list_Cargos.value = [
    {
      value: 1,
      cargo: "MR",
      label: `${tab.value == "DIP" ? "Diputaciones" : "Regidurías"
        } mayoría relativa`,
    },
    {
      value: 2,
