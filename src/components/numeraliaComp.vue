<template>
  ...
  <banner>
    ...
    <template v-slot:contenido>
      <span id="speak">
        Consulta la información proporcionada de manera obligatoria por las
        candidaturas que participan en el proceso electoral local ordinario
        2024. La información es responsabilidad de los partidos políticos. El
        Instituto únicamente apoya para su difusión.
      </span>
    </template>
  </banner>
  <br />
  <!---------------------------CHARTS--------------------------->
  <div v-if="loading == true" class="q-pa-md example-row-equal-width">
    <div class="row flex-center text-h6">
      <q-spinner-cube color="pink" size="5.5em" />
    </div>
  </div>
  <div class="row" v-else>
    <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12 q-pr-lg">
    ...
    <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12 q-pr-lg">
      ...
      <div
        style="height: 500px"
        class="flex-center"
        v-if="loadingIdentidad == true"
      >
        <q-spinner-cube color="pink" size="5.5em" />
      </div>
      <chartGradoAcademico v-else class="flex flex-center" />
    </div>
  ...
  <q-splitter v-else unit="px">
    <template v-slot:before v-if="!$q.screen.xs && !$q.screen.sm">
      <q-tabs
        v-model="tab"
        vertical
        class="text-purple"
  ...
  <banner>
    ...
    <template v-slot:contenido>
      <span id="speak">
        ...
        que no implica necesariamente que hayan sido registradas en el marco de
        una acción afirmativa. Para mayor información sobre este último punto,
...
const tab = ref("Candidaturas indígenas");
const open_tab = ref(false);
const splitterModel = ref(
  !$q.screen.xs && !$q.screen.sm && !$q.screen.md ? 340 : 0
);
// "Registro curricular",
const tabs = ref([
